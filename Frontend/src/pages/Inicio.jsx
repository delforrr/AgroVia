...
function EstadoChip({ estado }) {
    ...
    return (
        <Chip size="small" label={estado}
            sx={{ fontWeight: 700, fontSize: '0.7rem', color: cfg.color, bgcolor: cfg.bg, border: `1px solid ${cfg.color}30` }}
        />
    );
}
...
export default function InicioPage() {
    ...
    useEffect(() => {
        ...
        getAvisos()
            .then((data) => setAvisos(data.slice(0, 4)))
            .catch(console.error)
    ...
    const opActivas = operaciones.filter(
        (o) => ![ESTADOS_OPERACION.CERRADA, ESTADOS_OPERACION.CANCELADA].includes(o.estado)
    ).slice(0, 3);
                ...
                <Grid container spacing={2.5}>
                    ...
                    <Grid size={{ xs: 12, md: 3 }}>
                        <Stack spacing={2}>
                            ...
                            {/* Clima */}
                            <Paper elevation={0} sx={{ borderRadius: 4, p: 2, border: '1px solid rgba(0,0,0,0.08)' }}>
                                <Typography variant="subtitle2" fontWeight={700} color="text.secondary" textTransform="uppercase" letterSpacing="0.05em" mb={1.5}>
                                    Clima — Córdoba
                                </Typography>
                                <Stack spacing={1.5}>
                                    {CLIMA_MOCK.map((c) => (
                                        <Stack key={c.dia} direction="row" justifyContent="space-between" alignItems="center">
                                            <Stack direction="row" spacing={1} alignItems="center">
                                                {c.icon}
                                                <Box>
                                                    <Typography variant="body2" fontWeight={600}>{c.dia}</Typography>
                                                    <Typography variant="caption" color="text.secondary">{c.desc} · 💧{c.humedad}</Typography>
                                                </Box>
                                            </Stack>
                                            <Typography variant="h6" fontWeight={700} color="text.primary">{c.temp}</Typography>
                                        </Stack>
                                    ))}
                                </Stack>
                            </Paper>
                            ...
                            <Paper elevation={0} sx={{ borderRadius: 4, p: 2, border: '1px solid rgba(0,0,0,0.08)' }}>
                                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1.5}>
                                    <Stack direction="row" spacing={0.75} alignItems="center">
                                        ...
                                        <Typography variant="subtitle2" fontWeight={700} color="text.secondary" textTransform="uppercase" letterSpacing="0.05em">
                                            Alertas
                                        </Typography>
                                    </Stack>
                                    <Chip size="small" label={NOTIFS.length} color="primary" sx={{ height: 18, fontSize: '0.68rem' }} />
                                </Stack>
                                <Stack spacing={1.5}>
                                    {NOTIFS.map((n) => (
                                        <Box key={n.id} sx={{
                                            p: 1.25, borderRadius: 2,
                                            bgcolor: n.tipo === 'warning' ? '#fff8e1' : '#f3f8ff',
                                            borderLeft: `3px solid ${n.tipo === 'warning' ? '#f9a825' : '#1976d2'}`,
                                        }}>
                                            <Typography variant="caption" color="text.primary" display="block" lineHeight={1.4}>
                                                {n.texto}
                                            </Typography>
                                            <Typography variant="caption" color="text.secondary">hace {n.hace}</Typography>
                                        </Box>
                                    ))}
                                </Stack>
                            </Paper>
                    ...
                    <Grid size={{ xs: 12, md: 9 }}>
                        <Stack spacing={2.5}>
                            ...
                            <Paper elevation={0} sx={{ borderRadius: 4, border: '1px solid rgba(0,0,0,0.08)', overflow: 'hidden' }}>
                                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 2.5, pt: 2, pb: 1.5 }}>
                                    <Stack direction="row" spacing={1} alignItems="center">
                                        ...
                                        <Chip size="small" label="Granos · Rosario" variant="outlined" sx={{ fontSize: '0.68rem' }} />
                                    </Stack>
                                    <Stack direction="row" spacing={1} alignItems="center">
                                        <Tooltip title="Actualizar">
                                            <IconButton size="small" onClick={cargarCotizaciones} disabled={cotizLoading}
                                ...
                                <Grid container sx={{ px: 0.5, py: 1 }}>
                                    {cotizLoading
                                        ...
                                        : granosDestacados.map((g, i) => (
                                            <Grid key={g.id} size={{ xs: 6, sm: 3 }}
                                                sx={{
                                                    p: 1.5,
                                                    borderRight: i < 3 ? '1px solid rgba(0,0,0,0.06)' : 'none',
                                                    '&:nth-of-type(2)': { borderRight: { xs: 'none', sm: '1px solid rgba(0,0,0,0.06)' } },
                                                }}
                                            >
                                    ...
                                    <>
                                        ...
                                        <Stack direction="row" flexWrap="wrap" gap={0} sx={{ px: 2, py: 1.5 }}>
                                            {cotiz.divisas.slice(0, 4).map((d) => (
                                                <Stack key={d.id} direction="row" spacing={0.75} alignItems="center" sx={{ mr: 2.5, my: 0.25 }}>
                                                    <Typography variant="caption" color="text.secondary" fontWeight={600}>{d.nombre}:</Typography>
                                                    <Typography variant="caption" fontWeight={700} color="text.primary">${fmt(d.precio)}</Typography>
                            ...
                            {/* Operaciones activas */}
                            <Paper elevation={0} sx={{ borderRadius: 4, border: '1px solid rgba(0,0,0,0.08)' }}>
                                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 2.5, pt: 2, pb: 1.5 }}>
                                    <Stack direction="row" spacing={1} alignItems="center">
                                        <HandshakeIcon sx={{ color: '#7b1fa2', fontSize: 20 }} />
                                        <Typography variant="subtitle1" fontWeight={700}>Operaciones activas</Typography>
                                        {!opLoading && (
                                            <Chip size="small" label={opActivas.length} sx={{ height: 18, fontSize: '0.68rem', bgcolor: '#f3e5f5', color: '#7b1fa2' }} />
                                        )}
                                    </Stack>
                                    <Button size="small" endIcon={<ArrowForwardIcon />}
                                ...
                                {opLoading ? (
                                    ...
                                    <Box sx={{ textAlign: 'center', py: 4 }}>
                                        <Typography variant="body2" color="text.secondary">No tenés operaciones activas</Typography>
                                    </Box>
                                ) : (
                                    <Stack spacing={0} divider={<Divider />}>
                                        {opActivas.map((op) => {
                                            ...
                                            return (
                                                <Box key={op.id} sx={{ px: 2.5, py: 1.75, cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' }, transition: 'background 0.15s' }}
                                                    onClick={() => navigate('/operaciones')}>
                                                    <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1}>
                                                        <Box sx={{ flex: 1, minWidth: 0 }}>
                                                            <Stack direction="row" spacing={1} alignItems="center" mb={0.5} flexWrap="wrap">
                                                                <Chip
                                                                    size="small"
                                                                    icon={op.tipo === 'Granos' ? <GrainIcon /> : op.tipo === 'Hacienda' ? <PetsIcon /> : <AgricultureIcon />}
                                                                    label={op.tipo}
                                                                    variant="outlined"
                                                            ...
                                                            <Typography variant="body2" fontWeight={600} noWrap>{op.titulo}</Typography>
                                                        </Box>
                                                        <Typography variant="body2" fontWeight={700} color="primary.main" sx={{ ml: 1, whiteSpace: 'nowrap' }}>
                                                        ...
                                                        <Box sx={{ flexGrow: 1 }}>
                                                            <LinearProgress
                                                                ...
                                                                sx={{
                                                                    ...
                                                                    '& .MuiLinearProgress-bar': { bgcolor: pct === 100 ? '#2e7d32' : 'primary.main' },
                                                                }}
                                                            />
                                                        </Box>
                            ...
                            <Paper elevation={0} sx={{ borderRadius: 4, border: '1px solid rgba(0,0,0,0.08)' }}>
                                ...
                                {avisosLoading ? (
                                    ...
                                    <Stack divider={<Divider />}>
                                        {avisos.map((av) => (
                                            <Stack key={av.id} direction="row" spacing={2} alignItems="center"
                                                sx={{ px: 2.5, py: 1.5, cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' }, transition: 'background 0.15s' }}
                                                onClick={() => navigate('/avisos')}>
                                                <Avatar
                                                    sx={{
                                                        width: 44, height: 44, borderRadius: 2,
                                                        bgcolor: av.categoria === 'Hacienda' ? '#e8f5e9' : av.categoria === 'Maquinaria' ? '#e3f2fd' : '#fff8e1',
                                                        fontSize: '1.3rem',
                                                    }}
                                                >
                                                    {av.categoria === 'Hacienda' ? '🐄' : av.categoria === 'Maquinaria' ? '🚜' : av.categoria === 'Granos' ? '🌾' : '🔧'}
                                                </Avatar>
                                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                                    <Typography variant="body2" fontWeight={600} noWrap>{av.titulo}</Typography>
                                                    <Stack direction="row" spacing={0.75} alignItems="center" flexWrap="wrap">
                                                        <Chip size="small" label={av.categoria}
